...
*
* Portions Copyright (c) 2010-2011 - cocos2d-x community
* 
...
inline void  HeyaldaThrowIfFailed(HRESULT hr)
{
 ...
 HeyaldaMediaStreamer:: HeyaldaMediaStreamer() :
    m_offset(0)
{
...
Platform::Array<byte>^  HeyaldaMediaStreamer::ReadData(
...
void  HeyaldaMediaStreamer::Initialize(__in const WCHAR* url)
{
	...
	DWORD chunkPos = 0;
	 ...
	 HeyaldaThrowIfFailed(ReadChunk(MAKEFOURCC('R', 'I', 'F', 'F'), chunkSize, chunkPos));
	if (*reinterpret_cast<const DWORD *>(&dataPtr[chunkPos]) != MAKEFOURCC('W', 'A', 'V', 'E'))  HeyaldaThrowIfFailed(E_FAIL);

	// Locate 'fmt ' chunk, copy to WAVEFORMATEXTENSIBLE.
	 HeyaldaThrowIfFailed(ReadChunk(MAKEFOURCC('f', 'm', 't', ' '), chunkSize, chunkPos));
	 HeyaldaThrowIfFailed((chunkSize <= sizeof(m_waveFormat)) ? S_OK : E_FAIL);
	CopyMemory(&m_waveFormat, &dataPtr[chunkPos], chunkSize);

	// Locate the 'data' chunk and copy its contents to a buffer.
	 HeyaldaThrowIfFailed(ReadChunk(MAKEFOURCC('d', 'a', 't', 'a'), chunkSize, chunkPos));
...
void  HeyaldaMediaStreamer::ReadAll(uint8* buffer, uint32 maxBufferSize, uint32* bufferLength)
{
...
void  HeyaldaMediaStreamer::Restart()
{
	m_offset = 0;
}
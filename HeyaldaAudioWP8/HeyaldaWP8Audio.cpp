...
#include "HeyaldaMediaStreamer.h"
#include "cocos2d.h"

inline void  HeyaldaThrowIfFailed(HRESULT hr)
{
    if (FAILED(hr))
    {
...
void HeyaldaAudioEngineCallbacks::Initialize(HeyaldaWP8Audio *audio)
{
    m_audio = audio;
}
...
void  _stdcall HeyaldaAudioEngineCallbacks::OnCriticalError(HRESULT Error)
{
    UNUSED_PARAM(Error);
    m_audio->SetEngineExperiencedCriticalError();
};

 HeyaldaWP8Audio::HeyaldaWP8Audio() :
    m_backgroundID(0),
	m_soundEffctVolume(1.0f),
	m_backgroundMusicVolume(1.0f),
    m_maxFrequencyRatio(2.0)

{}

void  HeyaldaWP8Audio::Initialize()
{
...
void  HeyaldaWP8Audio::CreateResources()
{
    try
    {	
         HeyaldaThrowIfFailed(
	    ...
	    // This sample plays the equivalent of background music, which we tag on the mastering voice as HeyaldaAudioCategory_GameMedia.
	    ...
	    // our sound effects with the audio category HeyaldaAudioCategory_GameEffects.
	     HeyaldaThrowIfFailed(
		...
		m_soundEffectEngine->SetDebugConfiguration(&debugConfig);
...
unsigned int  HeyaldaWP8Audio::Hash(const char *key)
{
...
void  HeyaldaWP8Audio::ReleaseResources()
{
...
void  HeyaldaWP8Audio::PlayBackgroundMusic(const char* pszFilePath, bool bLoop)
{
...
void  HeyaldaWP8Audio::StopBackgroundMusic(bool bReleaseData)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::PauseBackgroundMusic()
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::ResumeBackgroundMusic()
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::RewindBackgroundMusic()
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
bool  HeyaldaWP8Audio::IsBackgroundMusicPlaying()
{
    return IsSoundEffectStarted(m_backgroundID);
}

void  HeyaldaWP8Audio::SetBackgroundVolume(float volume)
{
...
float  HeyaldaWP8Audio::GetBackgroundVolume()
{
    return m_backgroundMusicVolume;
}

void  HeyaldaWP8Audio::SetSoundEffectVolume(float volume)
{
	...
	for (iter = m_soundEffects.begin(); iter != m_soundEffects.end(); iter++)
	{
        if (iter->first != m_backgroundID)
            iter->second.m_soundEffectSourceVoice->SetVolume(m_soundEffctVolume * iter->second.m_soundEffectVolume);
	}
}

float  HeyaldaWP8Audio::GetSoundEffectVolume()
{
...
{
    unsigned int sound;
    this->PlaySoundEffect(pszFilePath, bLoop, sound, false);
	this->setEffectVolume(sound, gain * m_soundEffctVolume);
    this->setEffectPitch(sound, pitch);
    // TODO: Add pan support: http://msdn.microsoft.com/en-us/library/windows/desktop/hh405043(v=vs.85).aspx
    return sound;
}

void  HeyaldaWP8Audio::setEffectPitch(unsigned int sound, float pitch)
{
	if (m_engineExperiencedCriticalError) {
		cocos2d::log("m_engineExperiencedCriticalError == true");
		return;
	}

	if (m_soundEffects.count(sound)){
		HeyaldaSoundEffectData effectData = m_soundEffects[sound];

		if (pitch < XAUDIO2_MIN_FREQ_RATIO)
		{
			pitch = XAUDIO2_MIN_FREQ_RATIO;
		}
		else if (pitch > m_maxFrequencyRatio)
		{
			pitch = m_maxFrequencyRatio;
		}
		effectData.m_soundEffectSourceVoice->SetFrequencyRatio(pitch);
		//cocos2d::log("Sound:%u Pitch:%.2f", sound, pitch);
	}
}
void  HeyaldaWP8Audio::setEffectVolume(unsigned int sound, float volume)
{
	if (m_engineExperiencedCriticalError) {
		return;
	}
	...
	if (m_soundEffects.count(sound)){
		...
		HeyaldaSoundEffectData effectData = m_soundEffects[sound];
		effectData.m_soundEffectSourceVoice->SetVolume(m_soundEffctVolume * volume);
		effectData.m_soundEffectVolume = volume;
		//cocos2d::log("Sound:%u volume:%.2f", sound, volume);

	}
}

void  HeyaldaWP8Audio::PlaySoundEffect(const char* pszFilePath, bool bLoop, unsigned int& sound, bool isMusic)
{
...
void  HeyaldaWP8Audio::PlaySoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
	...
	HeyaldaSoundEffectData* soundEffect = &m_soundEffects[sound];
...
void  HeyaldaWP8Audio::StopSoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::PauseSoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::ResumeSoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::RewindSoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::PauseAllSoundEffects()
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::ResumeAllSoundEffects()
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::StopAllSoundEffects(bool bReleaseData)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
bool  HeyaldaWP8Audio::IsSoundEffectStarted(unsigned int sound)
{
    if (m_soundEffects.end() == m_soundEffects.find(sound))
        return false;
...
void  HeyaldaWP8Audio::PreloadSoundEffect(const char* pszFilePath, bool isMusic)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
    ...
    int sound = Hash(pszFilePath);
	 ...
	 HeyaldaMediaStreamer mediaStreamer;
    ...
    if (isMusic)
    {
	    ...
	    m_musicEngine->CreateSourceVoice(&m_soundEffects[sound].m_soundEffectSourceVoice,
            &(mediaStreamer.GetOutputWaveFormatEx()), 0, m_maxFrequencyRatio, &m_voiceContext, &sends)
	    );
...
void  HeyaldaWP8Audio::UnloadSoundEffect(const char* pszFilePath)
{
    int sound = Hash(pszFilePath);
...
void  HeyaldaWP8Audio::UnloadSoundEffect(unsigned int sound)
{
    if (m_engineExperiencedCriticalError) {
        return;
    }
...
void  HeyaldaWP8Audio::RemoveFromList( unsigned int sound )
{
